...
class PermissionService
{
    ...
    {
        $permissions = $this->get_tree($permission);
    ...
    public function getPermissionMenu($id) :array
    {
        ...
        if(auth('admin')->user()->name == 'Administrator') {

            $query = Permission::query()->where('status', 1)->where('is_menu',1);
            ...
            $permissionsMenu = $this->get_tree($permissions);

            return [$permissionsMenu, $permissions];
        } else {
            if(!empty($permissions)) {
                $query = Permission::query()->with('getPid')->where('status', 1)->where('is_menu',1);
                ...
                $permissionsMenu = $this->get_tree($permissions_array);
    ...
    public function setPermissions($nodeId,$id)
    {
        ...
        $permissions = Permission::query()->with('getPid')->where('status', 1)
            ->where('p_id','<>',0)
            ->whereIn('id', $nodeId)
            ->groupBy('id')
